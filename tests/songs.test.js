...
import { getAllArtists, getSongsFromArtist, orderAlphabetically, orderByYear, songsByGenre, minutsToSeconds, getLongestSong, getShortestSong } from '../src/songs';
...
describe('function "getShortestSong"', () => {
    ...
    it('should return an array of objects that share the same duration', () => {
        const arr = [
            { title: "Lust For Life", duration: "5min 12sec" },
            { title: "Here Comes Your Man", duration: "3min 20sec" },
            { title: "Gotta Go", duration: "3min 20sec" }
        ];
        expect(getShortestSong(arr)).toEqual([
            { title: "Here Comes Your Man", duration: 200 },
            { title: "Gotta Go", duration: 200 }
        ]);
    });
});